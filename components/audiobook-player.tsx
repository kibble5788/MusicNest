...
import type { Audiobook, Chapter, AudiobookPlayProgress } from "@/types/audiobook"
...
export default function AudiobookPlayer({ bookId, chapterId, onBack, setHideNavigation }: AudiobookPlayerProps) {
  // 状态
  ...
  useEffect(() => {
    const fetchAudiobookDetail = async () => {
      try {
        ...
        if (!targetChapterId && response.book.chapters && response.book.chapters.length > 0) {
  ...
  // 加载章节
  const loadChapter = async (bookId: string, chapterId: string, initialProgress = 0) => {
    try {
      setIsLoading(true)
      setError(null)

      const response = await audiobookApiClient.getChapterDetail(bookId, chapterId)
  ...
  const savePlayProgress = useCallback(() => {
    ...
    audiobookApiClient.savePlayProgress(progress).catch((err) => console.error("保存播放进度失败:", err))
  ...
  const handlePreviousChapter = () => {
    ...
    const currentIndex = audiobook.chapters.findIndex((c) => c.id === currentChapter.id)
    if (currentIndex > 0) {
          ...
          {audiobook.chapters.map((chapter) => (
            ...
            >
              <div className="flex items-center">
                {currentChapter?.id === chapter.id && <div className="w-2 h-2 bg-emerald-500 rounded-full mr-2"></div>}
                <div>
                  <h3 className="text-sm font-medium">{chapter.title}</h3>
                  <p className="text-xs text-gray-400">{formatTime(chapter.duration)}</p>
                </div>
              </div>

              {chapter.isLocked && <div className="text-xs bg-amber-500/20 text-amber-500 px-2 py-1 rounded">VIP</div>}
            </div>
          ))}
        </div>
      </div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {[0.5, 0.75, 1, 1.25, 1.5, 1.75, 2].map((rate) => (
                <Button
                  ...
                  className={playbackRate === rate ? "bg-emerald-500 text-black" : ""}
            ...
            <div className="grid grid-cols-4 gap-2">
              {[15, 30, 45, 60, 90, 120].map((minutes) => (
                <Button
                  ...
                  className={sleepTimer === minutes ? "bg-emerald-500 text-black" : ""}
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white flex flex-col">
      ...
      <div className="sticky top-0 z-10 backdrop-blur-md">
        <div className="flex items-center justify-between p-4">
          ...
          <h1 className="text-lg font-medium truncate flex-1 text-center">{audiobook?.title || "有声书播放器"}</h1>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex-1 flex items-center justify-center py-8">
              <div className={cn("w-64 h-64 rounded-lg overflow-hidden shadow-lg", isPlaying && "animate-pulse-scale")}>
                <img
                  src={audiobook?.cover || "/placeholder.svg?height=400&width=400&query=audiobook+cover"}
            ...
            <div className="mb-8 text-center">
              <h2 className="text-xl font-bold mb-2">{currentChapter?.title || "未知章节"}</h2>
              <p className="text-gray-400">
                {audiobook?.author.name || "未知作者"} · {audiobook?.narrator || "未知播音"}
              </p>
            </div>
            ...
            <div className="flex items-center justify-center space-x-8 mb-8">
              <Button variant="ghost" size="icon" onClick={handlePreviousChapter} className="text-white">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <Button variant="ghost" size="icon" onClick={() => setShowChapterList(true)}>
            ...
            {/* 播放速率显示 */}
            {playbackRate !== 1 && (
              <div className="text-center text-sm text-emerald-500 mb-4">播放速度: {playbackRate}x</div>
            )}
          </>
        )}
      </div>
