...
export default function Music({ setHideNavigation }: MusicProps) {
  ...
  const fetchData = async (forceRefresh = false) => {
    try {
      ...
      const [musicData, playlistsData] = await Promise.all([
        getMusicData(forceRefresh),
        getPlaylists(forceRefresh),
      ]);
  ...
  return (
    <div>
      ...
      <div className="space-y-6 pb-20 overflow-y-auto h-[calc(100vh-100px)]">
          ...
          {data?.featuredCards.map((card) => (
            ...
            >
              <div className="space-y-1">
                ...
                <p className="text-black/70 text-xs line-clamp-1">
                  {card.subtitle}
                </p>
              </div>
