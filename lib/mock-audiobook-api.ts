...
const generateCoverUrl = (id: string, title: string) => {
  return `/placeholder.svg?height=400&width=400&query=audiobook+${encodeURIComponent(
    title
  )}`;
};
  ...
  return Array(count)
    ...
    .map((_, index) => {
      ...
      return {
        ...
        description: `这是《${title}》的有声书版本，由专业播音员朗读。${
          Math.random() > 0.5
            ? "这部作品讲述了一个关于人性、命运和选择的故事。"
            : "这是一部引人入胜的作品，将带您进入一个奇妙的世界。"
        }`,
        author,
        narrator: ["张震", "林兆铭", "刘琳", "李野墨", "周建龙"][
...
export const audiobookApi = {
  ...
  getChapterDetail: async (bookId: string, chapterId: string) => {
    ...
    const chapters =
      book.chapters || generateChapters(bookId, book.totalChapters);
  ...
  savePlayProgress: async (progress: AudiobookPlayProgress): Promise<void> => {
    ...
    try {
      const savedProgress = JSON.parse(
        localStorage.getItem("audiobook_progress") || "{}"
      );
  ...
  getPlayProgress: async (
    bookId: string
  ): Promise<AudiobookPlayProgress | null> => {
...
export const audiobookApiClient = {
  ...
  getChapterDetail: (bookId: string, chapterId: string) => {
    return axiosInstance
      .get(`/audiobooks/${bookId}/chapters/${chapterId}`)
      .then((response) => response.data);
  },

  savePlayProgress: (progress: AudiobookPlayProgress) => {
    return axiosInstance
      .post("/audiobooks/progress", progress)
      .then((response) => response.data);
  },

  getPlayProgress: (bookId: string) => {
    return axiosInstance
      .get(`/audiobooks/${bookId}/progress`)
      .then((response) => response.data);
  },
};
    ...
    if (config.url?.startsWith("/audiobooks")) {
      try {
        ...
        if (config.url === "/audiobooks" && config.method === "get") {
          ...
          config.url.match(/\/audiobooks\/[^/]+$/) &&
          config.method === "get"
        ) {
          const id = config.url.split("/").pop() || "";
          response = await audiobookApi.getAudiobookDetail(id);
        } else if (
          config.url.match(/\/audiobooks\/[^/]+\/chapters\/[^/]+$/) &&
          config.method === "get"
        ) {
          ...
          config.url === "/audiobooks/progress" &&
          config.method === "post"
        ) {
          await audiobookApi.savePlayProgress(config.data);
          response = { success: true };
        } else if (
          config.url.match(/\/audiobooks\/[^/]+\/progress$/) &&
          config.method === "get"
        ) {
